...
tree_Dir <- 'mkdir output_tree/scenario3'
if (!file.exists('output_tree/scenario3')){
  system(tree_Dir)
}

seq_Dir <- 'mkdir output_seq/scenario3'
if (!file.exists('output_seq/scenario3')){
  system(seq_Dir)
}


for(n in 1:N){
  command_tree = paste('./ms ',M,' 1 -T -seeds ',n,' 10 10 | tail +4 | grep -v //>output_tree/scenario3/tree',n,'.newick', sep = '')
  system(command_tree)
  command_seq = paste('./ms ',M,' 1  -s ', L,' -seeds ',n,' 10 10 | tail +7 | grep -v //>output_seq/scenario3/seq',n,'.txt', sep = '')
  system(command_seq)
}

#readChar(file = 'output_seq/seq1.txt', header = FALSE)
for(i in 1:N){
  readseq_dir <- paste('output_seq/scenario3/seq',i ,'.txt', sep = '')
  ...
  filename = paste('output_seq/scenario3/K_', i, '.csv', sep = "")
...
for(i in 1:N){
  tree_name = paste("output_tree/scenario3/tree", i, ".newick", sep = '')
...
reftree_name = "output_tree/scenario3/reftree.newick"
tree.anc <- read.tree(tree_name)
