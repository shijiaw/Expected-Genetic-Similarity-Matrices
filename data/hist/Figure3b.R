...
df = data.frame(dif = dif[Difference == 'G-S'], Difference = Difference[Difference == 'G-S'])

g1 <- ggplot(df, aes(x=dif, fill = Difference, color = Difference)) +
  geom_histogram(aes(y = (..count..)/sum(..count..)), position="identity", binwidth=0.01, colour = "#00ba45",
     fill="#00ba45")+ xlab("") + ylab("Proportion")+ theme_minimal()+ xlim(-1.5, 0.5) + ylim(0,0.25) +
  theme(legend.position="none") + ggtitle('Kernel Method -vs- Ground Truth')

ggsave(file = 'Figure3b.pdf', plot = ggplot2::last_plot(), height = 2,
